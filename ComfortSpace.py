...
alt_colormap = cm.get_cmap('inferno', 100)
...
current_instrument = 85 # 77 and 85, 90, 96, 113 -1 usually
player.set_instrument(current_instrument) # 45

notes = [False for n in range(128)]
transmat = np.zeros((12, 12)).astype('float')

N = 100
points = np.random.random((N, 2)) # 100 points.
point_notes = np.asarray([q%12 for q in range(N)])
...
def sim():
    ...
    while True:
        ...
        multmat = np.ones((N,N)) - np.identity(N)*0.5
        for x in range(N):
            for y in range(N):
                multmat[x][y] = 1 + transmat[point_notes[x]][point_notes[y]]

        multmat = np.log1p(multmat)
        ...
        x_motion = C_distances.ravel() * x_differences * 0.002
        y_motion = C_distances.ravel() * y_differences * 0.002
...
while True:
    ...
    if i.poll():
        ...
        for e in midi_evs:
            ...
            if e.data2 > 0 and e.data1 in [36, 38]:
                if e.data1 == 36:
        ...
        for m, note_2 in enumerate(notes):
            if note_1 and note_2:
             #transmat[n%12][m%12] = (transmat[n%12][m%12] + 0.1) * 1.01
                ...
                transmat[n%12][m%12] += 0.5# min(1, transmat[n%12][m%12] + 0.04)
    ...
    for pc in range(len(points)):
        p = points[pc]
        col = colors[pc%12] if lights[pc%12] else (80,80,80) #[int(n/5) for n in colors[pc%12]] # (60, 60, 60)
        pygame.draw.rect(screen, col, (300+ int(40*p[0]), 250 + int(40*p[1]), 4, 4), 0)
